...
const Home = () => {
  ...
  const fetchNFTs = async() => {
    ...
    const api_key = "ataC1X6irCily57LGF3yL3E0Nqe9tGK6";
    const baseURL = `https://eth-mainnet.alchemyapi.io/v2/${api_key}/getNFTs/`;
    var requestOptions = {
      method: 'GET'
    };

    //if collection length is empty
    if (!collection.length) { 
      const fetchURL = `${baseURL}?owner=${wallet}`;
      nfts = await fetch(fetchURL, requestOptions).then(data => data.json());
    } else {
      console.log("fetching nfts for collection owned by address");
      // %5B%5D string right after the contract address specifies that the contract address parameter is an array and not a simple string.
